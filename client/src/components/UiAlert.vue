...
<template>
    <div v-if="alert.show" class="alert p-4 rounded-4" :class="alerts[props.alert.data.type].style" role="alert">
        <div class="alert-heading d-flex justify-content-between align-items-center mb-2">
            <h4 class="p-0 m-0"><i class="me-1 bi" :class="alerts[props.alert.data.type].icon"></i> {{
                alerts[props.alert.data.type].title }}</h4>
        ...
        <div class="progress mt-3" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"
            style="height: 1px">
