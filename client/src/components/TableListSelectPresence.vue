<script setup>
import { ref, watch } from 'vue'
import utils from '@/utils/utils';
import TableListStatus from './TableListStatus.vue'

const props = defineProps({
    sent: { type: Boolean, default: () => false },
    header: { type: Array },
    body: { type: Array, default: () => [] },
    casts: { type: Object },
    count: { type: Boolean, default: () => true },
    identify: { type: String },
})

const model = defineModel({ default: [] })
const body = ref(props.body)

function orderBy(key) {
    body.value.sort((a, b) => {
        if (typeof a[key] === 'string') {
            return a[key].localeCompare(b[key])
        }

        return a[key] - b[key]
    })
}

function getdata(data, obj, key, cast = null, subject = 'id') {
    const value = obj ? data[obj][key] : data[key] ?? '';

    if (cast && props.casts && props?.casts[key]) {
        const datacast = (props.casts[key]).find(obj => obj[subject] === value) ?? {}
        return datacast[cast] ?? ''
    }

    return value
}

function matchUtils(str, utilsList = []) {
    let aux = str
    utilsList.forEach((util) => {
        switch (util) {
            case 'html':
                aux = utils.stripHTML(str)
                break
            case 'truncate':
                aux = utils.truncate(str, 200)
                break
        }
    })

    return aux
}

function checkTheBox(e) {
    e.currentTarget.firstChild.firstChild.click()
}

function findModelById(item) {
    return model.value.some((x) => x.id == item.id)
}

watch(() => props.body, (newValue) => {
    body.value = utils.reduceArrays(model.value, newValue)
});

const checkName = props.identify + '_check'

</script>

<template>
    <div v-if="props.sent">
        <p v-if="body.length && props.count" class="small txt-color-sec ps-5">
            <i class="bi bi-grip-vertical"></i> {{ (body.length).toString().padStart(2, '0') }} Registros Localizados
        </p>
        <div v-if="body.length" class="table-responsive-sm">
            <table class="table table-borderless table-hover m-0">
                <thead v-if="props.header">
                    <tr>
                        <th></th>
                        <th scope="col" v-for="h in props.header" :key="h.key" @click="orderBy(h.key)">
                            {{ h.title }} <i class="bi bi-arrow-down table-order-icon"></i>
                        </th>
                        <th v-if="props.actions" scope="col"></th>
                    </tr>
                </thead>
                <tbody v-if="body">
                    <tr @click="checkTheBox" v-for="b in body" :key="b.id" :class="{ 'active': findModelById(b) }">
                        <td class="align-middle" :class="findModelById(b) ? 'text-danger' : 'text-success'">
                            <input class="form-check-input d-none" type="checkbox" :name="checkName" v-model="model"
                                :value="b" :id="b.id + checkName">
                            <ion-icon :name="findModelById(b) ? 'close-circle-outline' : 'checkmark-circle-outline'"
                                class="fs-4"></ion-icon>
                        </td>
                        <td v-for="h in props.header" :key="`${b.id}-${h.key}`" class="align-middle">
                            <TableListStatus v-if="h.key === 'status'" :data="getdata(b, h?.obj, h.key, h?.cast)" />
                            <template v-else>{{ getdata(b, h?.obj, h.key, h?.cast) }}</template>
                            <p v-if="h.sub" class="small txt-color-sec p-0 m-0">
                                <span v-for="s in h.sub" :key="s.key" class="inline-block small">
                                    {{ matchUtils(`${s.title ?? ''} ${getdata(b, s?.obj, s.key, s?.cast)}`,
                                        s?.utils) }}
                                </span>
                            </p>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
    <div v-else class="text-center">
        <ion-icon name="chatbox-ellipses-outline" class="fs-4"></ion-icon>
        <p class="p-0 m-0 form-text">Aplique o filtro para visualizar os resultados...</p>
    </div>
</template>

<style>
th {
    white-space: nowrap;
}

table td:nth-child(2) {
    white-space: nowrap;
}

.table tr th:first-child {
    padding-left: 50px;
}

.table tr td:first-child {
    padding-left: 50px;
}

.table tr td:last-child {
    padding-right: 4cap;
    text-align: start;
}

.table,
.table th,
.table td {
    background-color: transparent !important;
    color: var(--color-text);
}

.table th {
    cursor: pointer;
    font-weight: 600;
    font-size: small;
}

.table-order-icon {
    font-size: 0.6rem;
}

.table th:hover i {
    color: var(--color-base);
}

tr {
    cursor: pointer;
}

tr.active {
    background-color: var(--color-shadow);
}
</style>